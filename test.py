...
if st.button("BGMチャンネルを提案"):
    if not api_key:
        ...
        with st.spinner("作成中...数十秒～1分くらいかかります"):
            try:
                # 1. URL読み取り
                ...
                # 3. キーワード抽出
                keyword_prompt = f"""
                以下の店舗のウェブサイト情報から、このお店の「業態」や「ターゲット」、「内装」「雰囲気」を表す検索キーワードを日本語で5～10個、カンマ区切りだけで出力してください。
                【ウェブサイト情報】
                {page_text}
                """
                ...
                【厳格な執筆ルール（★必ず守ること）】
                - ★重要★：「〇〇と申します」「この度は～」「ウェブサイトを拝見し～」といった冒頭の自己紹介・挨拶・手紙風の導入文章は【一切禁止】です。絶対に書かないでください。
                - 出力の1行目は、必ず「### 🏢 お店プロファイル」の見出しから直接書き始めてください。
